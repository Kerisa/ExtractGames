...
namespace
{
    ...
    int DecodeKeyFileData2(PVOID PackData, const PACKIDX& Idx)
    {
        ...
        const uint32_t DecodeArray[256] = {
        ...
        while (p < pEnd)
        {
            ...
            uint64_t mm5 = *(uint64_t*)((uint8_t*)DecodeArray + (arr1Idx & 0x7f) * 8);
    ...
    int GetKeyBuf(const HANDLE hPack, const vector<PACKIDX>& Idx, PBYTE *ppKeyFile, PDWORD pdwKeyFileLen)
    {
        DWORD  R;
        int    ret = 0;
        ...
        for (auto& i : Idx)
        {
            if (std::wstring((wchar_t*)i.Name).rfind(L".key") != std::wstring::npos)
            {
                found = true;
                key = i;
                break;
        ...
        if (!found)
        {
            AppendMsg(L"封包中找不到 Key\r\n");
            return -1;
        }

        // 处理KeyFile
        PBYTE retKey = (PBYTE)VirtualAlloc(NULL, key.CompressLen, MEM_COMMIT, PAGE_READWRITE);
        if (!retKey)
        {
            AppendMsg(L"内存分配失败\r\n");
            ret = -2;
        ...
        SetFilePointer(hPack, key.Offset, NULL, FILE_BEGIN);
        ReadFile(hPack, retKey, key.CompressLen, &R, NULL);
        assert(key.IsEncrypted == 1 && key.IsCompressed == 0);
        ret = DecodeKeyFileData(retKey, key);

        *ppKeyFile = retKey;
        *pdwKeyFileLen = key.CompressLen;
    ...
    {
        DWORD dwunCompreLen, dwKeyFileLen, R;
        DWORD dwFileProcesses = 0;
        PBYTE unCompre, pKeyFile;
        int   ret = 0;
        wchar_t	UniName[MAX_PATH], MagBuf[MAX_PATH];


        ret = GetKeyBuf(hPack, Idx, &pKeyFile, &dwKeyFileLen);
        if (ret)
        {
            AppendMsg(L"Key获取失败\r\n");
            return ret;
        ...
        for (DWORD i = 0; i < Idx.size(); ++i)
        {
            ...
            if (Idx[i].IsEncrypted == 1)
                ...
                ret = DecodeKeyFileData2(PackData, Idx[i]);
...
int Entrance2(const wchar_t *CurDir, const wchar_t *PackName)
{
    HANDLE			hPack;
