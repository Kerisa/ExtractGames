 ...
 * 做的一些修改：
 *
 * 我在使用时遇到new运算符引发的bad_alloc，不知什么原因，然而我试着改了一下VirtualAlloc居然就好了= =
 * 于是我把 new/delete 都改了，当然，添加了 #include <windows.h>
 * 
 * CBG::FixPic 中当 i==Oh-1 且 Wi>Ow 时，writefrom会指向小于from的地址，引起错误，
 * 所以我也加了一段进去
 *
 * 错误信息改成了宽字节
 *
 * CBG::UncomRGBInfo 中 if ( SK > 64 ) break; 改成了 >= ，T004C0180数组的长度就是64
 *
 * CBG::Alpha 里出现指针越界所以我加了两对 __try/__except
 *
 * 最后一个是去掉了 CBG::Uncompress 中 "透明通道非LZ压缩" 并退出的分支，
 * 并把原来的一个判断条件 *(int*)ZeroBlock==1 放到调用CBG::Alpha前的判断里
 * 后来新增的输出图片，用windows自带或是IrfanView都是色块但用Universal Viewer Pro可以看到正常的图像，
 * 图像似乎没有问题所以我就把更改保留下来了
 *
 *
 * 以上修改均基于提取 [150529][Lump of Sugar]恋想リレーション 时候的情况
 * 15-07-31
 * Alisa
 */
...
static wchar_t error0[ ] = L"目前没有错误~" ;
static wchar_t error1[ ] = L"这不是一个CBG文件！" ;
static wchar_t error2[ ] = L"这不是符合类型的CBG文件！" ;
static wchar_t error3[ ] = L"和结果  或者  异或和  结果不匹配！" ;
static wchar_t error4[ ] = L"数据块压缩前大小与图像宽度校验不匹配！" ;
static wchar_t error5[ ] = L"该程序仅支持32位的位图解码！" ;
static wchar_t error6[ ] = L"透明通道非LZ压缩！" ;
static wchar_t *ErrorInfo = error0 ;
...
{
	if ( BmpData != 0 )
		VirtualFree(BmpData, 0, MEM_RELEASE);
...
bool CBG::Uncompress( char *FileBuf , int FileSize )
{
	...
	HuffmanNote *Trees = (HuffmanNote*) VirtualAlloc(0, sizeof(HuffmanNote)*0x17F, MEM_COMMIT, PAGE_READWRITE);  
	...
	if (BmpData) VirtualFree(BmpData, 0, MEM_RELEASE);
	BmpData = (char*)VirtualAlloc(0, Size+56, MEM_COMMIT, PAGE_READWRITE);
	memset( BmpData , 0 , Size ) ;

	if (*( int * )ZeroBlock == 1 && Head->ColourDeep == 32 )  Alpha( BmpData , ZeroBlock + 4 , FileSize - ( ZeroBlock - FileBuf - 4 ) ) ;     /* 解压 LZ压缩的透明通道 */
	unRGB( BmpData , Trees , &RootNote[ 0 ] ) ;   /* JPEG 100%质量 */
	
	VirtualFree(Trees, 0, MEM_RELEASE);
...
bool CBG::Alpha( char *WriteTo , char *data , unsigned int MaxData )
{
	...
	while( offset < Size )
	{
		...
		{
			__try{ *( WriteTo + offset ) = *( data + DataCount ) ;}
			__except(1){ return true;}
			offset += 4 ;
			DataCount++ ;
		}
...
bool CBG::unRGB( char *WriteTo , HuffmanNote *Tree , int *Root )
{
	...
	for ( i = 0 ; i < BlockCount ; i++ )
	{
		...
		short int *ss = (short int*) VirtualAlloc(NULL, BK.RGBSize*sizeof(short int), MEM_COMMIT, PAGE_READWRITE);

		memset( ss , 0 , BK.RGBSize * 2 ) ;
		UncomRGBInfo( ss , &BK , Tree , Tree + 0x1F , *Root , *( Root + 1 ) ) ;
		FuckLast( ss , &Mul[ 0 ] , &BK ) ;

		VirtualFree(ss, 0, MEM_RELEASE);
	}
	return true ;
}
...
{
	char *to = (s8*)VirtualAlloc(0, Ow * Oh * 4 +56, MEM_COMMIT, PAGE_READWRITE);//new char[ Ow * Oh * 4 +56 ] ;
